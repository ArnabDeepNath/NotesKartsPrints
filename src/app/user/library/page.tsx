          ...
          {loading ? (
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-5">
              {[...Array(10)].map((_, i) => (
                <div
                  key={i}
                  className="bg-white border border-gray-200 rounded-md aspect-[3/4] animate-pulse"
                />
              ))}
            </div>
          ) : books.length === 0 ? (
                ...
                {books.map((book: any, i: number) => (
                  ...
                  >
                    <Link href={`/books/${book.id}`}>
                      ...
                      <h3 className="text-sm font-medium text-[#232f3e] line-clamp-2 group-hover:text-[#e47911] transition-colors">
                        {book.title}
                      </h3>
